...
Create a `Page Task Table` before writing prompts. It is the control sheet for the whole detail-page set and must be built from the `Module Plan` in `page-planning-strategy.md`.
...
## Default Eight-Screen Rhythm

Use this rhythm unless the `Module Plan` selects a better count or the user specifies another count:
